...
function createVis() {
    ...
    function redraw(data, average){
        ...
        arcs
            .transition()
            .duration(dur)
            .attrTween("d", arcTween(newRadius));
    ...
    function arcTween(newRadius) {
        var radius = lastRadius;
        return function() {
            d3.select(this).transition().duration(dur).attrTween("d", function(d) {
                var i = d3.interpolate(radius, newRadius);
                return function(t) {
                    d.outerRadius = i(t);
